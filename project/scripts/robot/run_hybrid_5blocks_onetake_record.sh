#!/usr/bin/env bash
#
# Hybrid 5-block one-take dataset recorder launcher for SO-101.
# Records continuous Auto (macro approach/return) + Manual (teleop manipulation) episodes.
#
...
DATASET_NAME="${DATASET_NAME:-task1_hybrid_5blocks_v1}"
...
MACRO_GOTO_DURATION_S="${MACRO_GOTO_DURATION_S:-2.5}"
MACRO_RETURN_DURATION_S="${MACRO_RETURN_DURATION_S:-2.0}"
HOVER_Z_OFFSET_M="${HOVER_Z_OFFSET_M:-0.08}"

YOLO_MODEL_PATH="${YOLO_MODEL_PATH:-$LEROBOT_ROOT/project/models/yolo_block_detector/best.pt}"
GRASP_CALIBRATION="${GRASP_CALIBRATION:-$LEROBOT_ROOT/project/config/grasp_pixel_to_robot.json}"
...
echo "================================================================="
echo "🎬 SO-101 Hybrid 5-Block One-Take Recorder"
echo "================================================================="
