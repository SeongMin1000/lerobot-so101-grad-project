        ...
        if target_xyz is not None and self.kin is not None:
            try:
                ...
                if orientation is not None:
                    ...
                    hover_xyz = target_xyz - h_xy_unit * h_offset + np.array([0.0, 0.0, 0.08])
                    approach_high_xyz = hover_xyz
                else:
                    hover_xyz = target_xyz + np.array([0.0, 0.0, 0.08])
                    approach_high_xyz = target_xyz + np.array([0.0, 0.0, 0.08])
...
def _record_one_take_5blocks_episode(
    *,
    ...
    for block_idx, color in enumerate(colors):
        ...
        if block_idx == 0:
            # 1st block: direct approach to +8cm hover pose (1-step smooth motion)
            waypoints = [
                (approach_high, cfg.macro_goto_duration_s),
                # (hover_target, cfg.macro_goto_duration_s * 0.4),  # 2차 움직임 주석 처리
            ]
            print(f"🤖 [AUTO] Moving to {color} hover pose from observe ({cfg.macro_goto_duration_s:.1f}s)...")
        else:
            # Subsequent blocks: lift vertically (+12cm clear) -> direct approach to +8cm hover pose
