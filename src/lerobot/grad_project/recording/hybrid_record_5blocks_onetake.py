...
"""Hybrid 5-block one-take dataset recorder for SO-101 VLA model training.

Key Features & Workflow:
========================
1. End-to-End Continuous Recording:
   - AUTO (macro approach/return) and MANUAL (teleoperation pick/place)
     are recorded seamlessly into a SINGLE episode without any frame drops.
2. 5-Block One-Take Sequence:
   - For each of the 5 blocks (e.g. red -> yellow -> wood -> green -> blue):
     a. [AUTO] Macro smoothly moves follower and leader to the target block's hover pose.
     b. [MANUAL] Leader torque is released (0 snapping!). Operator precisely descends,
        grasps, and places the block into the target slot/stack.
     c. [AUTO] Operator presses [SPACE] / [ENTER], leader torque engages, and arm smoothly
        returns to neutral/observe pose (or approaches next block).
3. Snapping-Free Handover:
   - In AUTO mode, leader arm actively tracks follower trajectory via torque-ON + feedback.
   - When switching to MANUAL, leader torque is released at the exact joint pose,
     preventing sudden jerky motions or snapping.
4. Clean Operator Controls:
   - [SPACE] / [ENTER] / [C]: Finish current block manual phase -> switch to AUTO next block.
   - [Right Arrow ->]: Commit & save the completed 5-block episode.
   - [Left Arrow <-]: Discard current episode buffer & restart at observe pose.
   - [ESC]: Discard and stop recording session.

Usage:
======
python -m lerobot.grad_project.recording.hybrid_record_5blocks_onetake \\
  --robot.type=so101_follower --robot.port=/dev/so101_follower \\
  --teleop.type=so101_leader --teleop.port=/dev/so101_leader \\
  --dataset.repo_id=eslab1234/task1_hybrid_5blocks_v1 \\
...
RECORDER_BUILD = "2026-08-20-hybrid-5blocks-onetake-v1"
...
class TargetHoverResolver:
    ...
    def __init__(self, cfg: HybridOneTakeRecordConfig):
        ...
        try:
            ...
            # Load grasp orientation reference (A2 pregrasp point)
            self._load_grasp_orientation_ref("A2")
        except Exception as e:
            logging.debug("Kinematics / orientation ref init: %s", e)

    def cache_scene_blocks(self, top_img: np.ndarray | None) -> None:
        """Runs YOLO on the initial observe frame and caches all block 3D positions."""
        self._cached_block_coords.clear()
        if top_img is None or self.yolo_detector is None or self.grasp_homography is None:
            return
        try:
    ...
    def _load_grasp_orientation_ref(self, label: str = "A2") -> None:
        """Loads reference rotation matrix and azimuth angle from pregrasp point (e.g. A2)."""
        if self.kin is None:
            return
        for point in self.pregrasp_points:
            if point.get("label") == label:
                ...
                self._ref_orientation = pose[:3, :3].copy()
                self._ref_azimuth_rad = float(np.arctan2(pose[1, 3], pose[0, 3]))
                return
    ...
    def grasp_orientation_for(self, target_xyz_m: np.ndarray) -> np.ndarray | None:
        """Reference grasp orientation rotated around world Z to face the target block."""
        ...
        return rot_z @ self._ref_orientation

    def pixel_to_robot_xyz(self, px: float, py: float) -> np.ndarray:
        """Converts (px, py) to robot base XYZ (m) with parallax and backlash offsets."""
        ...
        # Apply radial compensation and gear backlash trim
        radial_offset = 0.0
        if radial_offset:
            radial = np.array([xyz[0], xyz[1], 0.0])
            norm = float(np.linalg.norm(radial))
            if norm > 1e-6:
                xyz = xyz + (radial / norm) * radial_offset
        return xyz + np.array([0.015, 0.015, 0.0])
    ...
    def solve_ik_multiseed(
        ...
        """Solves IK from multiple seeds (current, neutral, ref) to avoid local minima."""
        ...
        if target_xyz is not None and self.kin is not None:
            try:
                if orientation is not None:
                    ...
                    hover_xyz = target_xyz - approach_axis * self.cfg.hover_z_offset_m
                else:
                    hover_xyz = target_xyz + np.array([0.0, 0.0, self.cfg.hover_z_offset_m])

                approach_high_xyz = target_xyz + np.array([0.0, 0.0, self.cfg.transit_height_m])
...
def _record_auto_macro_trajectory(
    *,
    robot: Any,
    teleop: Any,
    ...
    """Interpolates macro movement from start_robot_pose to target_pose while recording every frame."""
    steps = max(2, int(duration_s * fps))
    dt = 1.0 / fps
    keys = [k for k in target_pose if k in start_robot_pose]
    ...
    for i in range(1, steps + 1):
        ...
        # Compute interpolated joint command
        macro_cmd = {k: (1.0 - alpha) * start_robot_pose[k] + alpha * target_pose[k] for k in keys}
...
def _record_auto_waypoints_trajectory(
    *,
    robot: Any,
    teleop: Any,
    start_robot_pose: dict[str, float],
    ...
    """Executes a sequence of (waypoint_pose, duration_s) segments continuously with full recording."""
    current_start = dict(start_robot_pose)
    for target_pose, duration_s in waypoints:
        res = _record_auto_macro_trajectory(
            robot=robot,
            teleop=teleop,
            start_robot_pose=current_start,
        ...
        if res is not None:
            return res
        current_start = dict(target_pose)
    return None
...
def _record_one_take_5blocks_episode(
    *,
    ...
    """Executes the full 5-block one-take sequence with +12cm Transit High flight."""
    ...
    for block_idx, color in enumerate(colors):
        ...
        # -------------------------------------------------------------
        # 1. AUTO APPROACH: +12cm Transit flight to block hover pose
        # -------------------------------------------------------------
        obs = robot.get_observation()
        ...
        if block_idx == 0:
            # 1st block: from observe -> approach_high (+12cm) -> hover_target (+8cm)
            waypoints = [
                (approach_high, cfg.macro_goto_duration_s * 0.6),
                (hover_target, cfg.macro_goto_duration_s * 0.4),
            ]
            print(f"🤖 [AUTO] Moving to {color} hover pose from observe ({cfg.macro_goto_duration_s:.1f}s)...")
        else:
            # Subsequent blocks: lift vertically (+12cm clear) -> fly at +12cm -> descend to hover (+8cm)
            ...
            total_dur = sum(d for _, d in waypoints)
            print(f"🤖 [AUTO] +12cm Transit flight to {color} hover pose ({total_dur:.1f}s)...")

        macro_res = _record_auto_waypoints_trajectory(
            robot=robot,
            teleop=teleop,
        ...
        # -------------------------------------------------------------
        # 3. IF LAST BLOCK: Final return to observe
        # -------------------------------------------------------------
        if block_idx == len(colors) - 1:
            cur_robot, _ = _read_current_poses(robot, teleop)
            clear_pose = resolver.resolve_lift_clear_pose(cur_robot)
            print(f"🤖 [AUTO] Final return to observe over {cfg.macro_return_duration_s:.1f}s...")
            ret_res = _record_auto_waypoints_trajectory(
                robot=robot,
                teleop=teleop,
                start_robot_pose=cur_robot,
                waypoints=[
                    (clear_pose, 0.6),
                    (observe_target, max(0.8, cfg.macro_return_duration_s - 0.6)),
                ],
                fps=fps,
                dataset=dataset,
                single_task=single_task,
