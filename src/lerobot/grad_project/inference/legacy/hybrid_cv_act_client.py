#!/usr/bin/env python3
"""
Hybrid OpenCV + FSM/script + ACT client for SO-101.

This is a drop-in alternative to lerobot.async_inference.robot_client.
It still uses the GPU policy_server for ACT inference, but ACT is only run during
short wrist-camera grasp windows.

High-level loop:
  observe pose -> top OpenCV detect -> nearest pregrasp script move
  -> short ACT grasp -> script move to target slot -> open gripper -> repeat

Start policy server on GPU PC first, then run this on the local robot PC.
"""
...
@dataclass
class HybridRobotClientConfig(RobotClientConfig):
    ...
    # ACT should only run briefly for wrist grasp.
    act_grasp_seconds: float = 1.6
    ...
    # If true, do OpenCV and pose selection but do not move motors or run ACT.
    dry_run: bool = False

    # Save detector overlays for debugging.
    debug_save_dir: str = ""

    # Verbose LeRobot async logs during ACT windows.
    verbose: bool = False


class HybridRobotClient:
    prefix = "hybrid_cv_act_client"
    logger = get_logger(prefix)

    def __init__(self, config: HybridRobotClientConfig):
        self.config = config
        ...
        self.logger.info("Initializing hybrid client to connect to server at %s", self.server_address)
        ...
        self.logger.info("Robot connected and hybrid client ready")
    ...
    # ---------------------------------------------------------------------
    # Queue / async ACT methods, adapted from robot_client.py
    # ---------------------------------------------------------------------
    ...
    def control_loop_action(self, verbose: bool = False) -> dict[str, Any] | None:
        ...
        if verbose:
            self.logger.info("ACT action #%s performed", timed_action.get_timestep())
        return performed
    ...
    def run_act_grasp_window(self, seconds: float) -> None:
        if self.config.dry_run:
            self.logger.info("[DRY RUN] would run ACT for %.2fs", seconds)
            return
        self.logger.info("Running ACT grasp window for %.2fs", seconds)
        self.clear_action_queue()
        ...
        self.clear_action_queue()
        self.logger.info("ACT grasp window done")
    ...
    def pick_and_place_one(self, index: int) -> bool:
        ...
        self.run_act_grasp_window(self.config.act_grasp_seconds)
        ...
        # ACT가 끝난 직후의 gripper 상태, 즉 블록을 잡고 있는 상태를 유지한 채 이동한다.
...
@draccus.wrap()
def hybrid_client(cfg: HybridRobotClientConfig) -> None:
    logging.info(pformat(asdict(cfg)))
    client = HybridRobotClient(cfg)
    if client.start():
        ...
        try:
            ...
            client.logger.info("Hybrid client stopped")
