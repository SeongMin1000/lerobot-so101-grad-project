...
from lerobot.async_inference.opencv_block_detector import (
    TopBlockDetector,
    _odd_kernel,
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="configs/opencv_detector.json",
    )
    parser.add_argument(
        "--colors",
    ...
    parser.add_argument("--preview-dir", default="assets/opencv/calibration")
    ...
    try:
        ...
        print("\n다음 실행:")
        print("bash ~/lerobot/scripts/grad_project.sh opencv-debug")
        return 0
    finally:
        capture.release()
