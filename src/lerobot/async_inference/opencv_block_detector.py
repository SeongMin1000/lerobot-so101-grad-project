...
from lerobot.async_inference.hybrid_paths import lerobot_root
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "frame_color": "bgr",
    "background_image": "assets/opencv/top_background.png",
    "hsv_ranges": {
    ...
    def _load_background(self, frame_shape: tuple[int, int, int]) -> np.ndarray:
        if self._background_bgr is None:
            ...
            if background is None:
                raise FileNotFoundError(
                    ...
                    "Run: bash ~/lerobot/scripts/grad_project.sh opencv-background"
                )
        ...
        if self._background_bgr.shape != frame_shape:
            raise ValueError(
                ...
                "Recapture the background with: bash ~/lerobot/scripts/grad_project.sh opencv-background"
            )
    ...
    def _validate_color_prototypes(self) -> None:
        ...
        if missing or invalid:
            ...
            raise RuntimeError(
                ...
                "Run: bash ~/lerobot/scripts/grad_project.sh opencv-colors"
            )
...
def main() -> int:
    ...
    config_path = (
        ...
        if args.config
        else _resolve_project_path("configs/opencv_detector.json")
    )
