...
from lerobot.async_inference.hybrid_paths import runtime_config_path
...
class RecordConfig:
    ...
    # Hybrid grasp dataset auto-reset options.
    runtime_config: str = str(runtime_config_path())
    pregrasp_label: str = "A1"
...
def _hybrid_load_pregrasp_pose(runtime_config: str, label: str) -> dict[str, float]:
    runtime = json.load(open(runtime_config, encoding="utf-8"))
    ...
    for item in runtime.get("pregrasp_points", []):
        if item.get("label") == label:
            ...
            return {k: float(v) for k, v in pose.items() if k.endswith(".pos")}
...
def _hybrid_goto_pregrasp(robot: Robot, teleop, runtime_config: str, label: str, duration_s: float, fps: int) -> None:
    target_pose = _hybrid_load_pregrasp_pose(runtime_config, label)
    target_pose = {k: v for k, v in target_pose.items() if k in robot.action_features}

    if not target_pose:
        raise ValueError(f"No valid robot action keys found for pregrasp '{label}'")

    print()
    print("=" * 70)
    print(f"[AUTO GOTO] {label} pregrasp")
    print("=" * 70)
    ...
    # pregrasp 이동 중 leader 토크 ON.
    # 이동이 끝나도 Enter 누르기 전까지 leader 토크를 유지한다.
    ...
    for i in range(1, steps + 1):
        ...
        for k, target in target_pose.items():
            start = robot_start.get(k, target)
            robot_cmd[k] = start + (target - start) * a
        ...
        if leader is not None and hasattr(leader, "send_feedback"):
            leader_cmd = {}
            for k, target in target_pose.items():
                start = leader_start.get(k, target)
                leader_cmd[k] = start + (target - start) * a
            leader.send_feedback(leader_cmd)

        precise_sleep(dt)

    print(f"[OK] reached {label} pregrasp")
    print("[WAIT] leader torque is still ON. Press ENTER to unlock leader and start recording.")
    print()
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                if cfg.wait_enter_before_episode:
                    print("\n" + "=" * 70)
                    print(f"[READY] episode {dataset.num_episodes}")
                    print("블럭 배치 확인하고, gripper 열려있는지 확인한 다음 ENTER 치면 촬영 시작")
                    print("종료하려면 Ctrl+C")
                    print("=" * 70)
                    input()
