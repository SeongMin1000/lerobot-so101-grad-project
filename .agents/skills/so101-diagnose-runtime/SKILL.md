...
* **현상**: 블록 색상 구분, 대략적인 위치 이동, 슬롯 복귀는 완벽하지만, 마지막에 블록을 집으려 할 때 1~2cm 빗겨나가 헛손질함.
* **원인**: **학습 Epoch 부족(Underfitting)**. 대규모 데이터셋(30만 프레임)에서 5만 스텝 학습 시 실질적으로 데이터셋을 2.6 Epoch만 반복한 수준임. 거시적 궤적(평균 궤적)만 학습되고 미세 파지 오차(L1 Loss ~0.067)가 수렴하지 않음.
* **해결책**:
  - 학습 스텝을 **100k ~ 150k steps (5~8 Epochs)**로 연장하여 L1 Loss를 0.04 이하로 수렴시킬 것.
  - 학습 시 Chunk Size를 60(`--policy.chunk_size=60 --policy.n_action_steps=60`)으로 고정.
  - 이미지 증강(`--dataset.image_transforms.enable=true`) 활성화.
